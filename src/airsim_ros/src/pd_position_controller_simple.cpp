...
void PIDPositionController::compute_control_cmd()
{
    ...
    double vyaw = atan2(curr_error_.y, curr_error_.x);
    ROS_WARN("the vyaw is: %f", vyaw);
    // if(curr_error_.x < 0)
    // {
    //     if(curr_error_.y < 0)
    //         vyaw -= M_PI;
    //     else
    //         vyaw += M_PI;
    //     ROS_WARN("the vyaw isxdfsz  fvx: %f", vyaw);
    // }
    ...
    vel_cmd_.twist.angular.z = params_.kp_yaw * atan2(vel_cmd_.twist.linear.y, vel_cmd_.twist.linear.x);
}
