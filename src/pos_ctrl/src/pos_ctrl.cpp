...
POSCtrl::POSCtrl(ros::NodeHandle *nh)
{
    ...
    airsim_circle_pose_sub = nh->subscribe("/airsim_node/drone_1/debug/circle_poses_gt", 1, &POSCtrl::circle_poses_gt_cb, this);
...
void POSCtrl::circle_poses_gt_cb(const airsim_ros::CirclePoses& circle_poses){
    for(int goalIdx = 0; goalIdx < 18; goalIdx++){
        if(goalIdx < 4){
        goalpath[goalIdx][0] = circle_poses.poses[goalIdx].position.x;
        goalpath[goalIdx][1] = circle_poses.poses[goalIdx].position.y;
        goalpath[goalIdx][2] = circle_poses.poses[goalIdx].position.z;
        goalpath[goalIdx][3] = circle_poses.poses[goalIdx].yaw;
        }
        else if(goalIdx == 4)
        {
            continue;
        ...
        {
            goalpath[goalIdx][0] = circle_poses.poses[goalIdx-1].position.x;
            goalpath[goalIdx][1] = circle_poses.poses[goalIdx-1].position.y;
            goalpath[goalIdx][2] = circle_poses.poses[goalIdx-1].position.z;
            goalpath[goalIdx][3] = circle_poses.poses[goalIdx-1].yaw;
        }

        ROS_WARN("goal:%f, %f, %f, %f", goalpath[goalIdx][0], goalpath[goalIdx][1], goalpath[goalIdx][2], goalpath[goalIdx][3]);
    }
}
